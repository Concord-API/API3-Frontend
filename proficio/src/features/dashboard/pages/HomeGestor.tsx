...
export function HomeGestor() {
  ...
  // Somente colaboradores (exclui Gestores e Diretores)
  const somenteColaboradores = useMemo(() => {
  ...
  const filteredColabs = useMemo(() => {
    if (setorFilter === 'all') return somenteColaboradores
    const equipeIds = equipes.filter(eq => eq.id_setor === setorFilter).map(eq => eq.id_equipe)
    return somenteColaboradores.filter(c => equipeIds.includes(c.id_equipe))
  }, [somenteColaboradores, equipes, setorFilter])
      ...
      <div className="space-y-6">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription className="invisible">.</CardDescription>
          </CardHeader>
          <CardContent>
